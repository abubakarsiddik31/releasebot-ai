...
class Settings(BaseSettings):
    ...
    GITHUB_REPO: str = Field(
        default="", description="Repository in format username/repo"
    )
    ...
    BREVO_SENDER_EMAIL: str = Field(
        default="", description="Brevo sender email address"
    )
    BREVO_SENDER_NAME: str = Field(default="", description="Brevo sender name")

    # Database configuration
    DATABASE_URL: str = Field(
        default="postgresql+asyncpg://postgres:postgres@localhost:5435/releasebot",
    ...
    def set_github_repo_parts(cls, v, values, field):
        if (
            ...
            and not v
            and "GITHUB_REPO" in values
            and values["GITHUB_REPO"]
        ):
